...
- Gotchas: repo principle is that `git pull` should live-update deployed config without re-running bootstrap except when the deployment shape changes, so prefer repo-backed symlink targets over copied runtime files. `home/.vim` is now a real symlink node to `../managed/vim` so the Vim tree can stay whole-directory-linked without special bootstrap metadata. Git prompt/completion helpers are intentionally resolved from the active system Git install rather than deployed from this repo.

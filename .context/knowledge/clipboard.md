...
- In zsh `viins`, bind Backspace (`^?` and `^H`) to `backward-delete-char`, not `vi-backward-delete-char`; zsh's native vi widget can refuse to delete before the current insert point after `Esc`, motion, and `i`. Bind Delete (`^[[3~`) to `delete-char` so Backspace deletes backward and Delete deletes forward without touching the platform clipboard.
...
- zsh bracketed paste normally highlights newly pasted text using `zle_highlight`'s default `paste:standout`. Keep `paste:none` in `zle_highlight` so pasted command text does not look selected until the next edit.
- Bash/Readline bracketed paste can display pasted text as an active region. Keep `enable-active-region off`; this disables the temporary display highlight while leaving bracketed paste enabled.

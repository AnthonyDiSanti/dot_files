...
- Non-Git completions: Bash loads system/package-manager `bash-completion` frameworks plus generated completions for selected installed commands such as `docker`, `gh`, and `kubectl`; zsh prepends Homebrew site-functions before `compinit` and only falls back to generated completions when no native command completion exists.

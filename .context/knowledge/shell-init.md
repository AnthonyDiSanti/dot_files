...
- Line-editor policy: Bash uses Readline bindings/settings directly in `rc.bash`; zsh uses native ZLE widgets and completion matcher/presentation styles in `rc.zsh`. Both bind Up/Down to typed-prefix history search and use case-/hyphen-insensitive completion matching where native support exists. Zsh also loads `zsh/complist` and trials `menu select=2` so ambiguous completions can be selected from a menu.
...
- Zsh completion presentation belongs in `rc.zsh` with other ZLE/completion styles, before `tool-support.zsh` runs `compinit`. Current policy keeps colored/verbose/grouped/described lists, directories first, and `zsh/complist` menu selection only when completion is ambiguous.
...
- `command -v tool` is not enough to prove shell integration support. Older or distro-packaged tools can exist on PATH but reject their shell generator subcommands; loader code should try the generator quietly, require non-empty output, and skip integration if it fails.

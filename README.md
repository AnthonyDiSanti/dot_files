...
`bootstrap.sh` interprets the tracked `home/` tree as a literal `$HOME` mirror
and symlinks each managed leaf back into the repo. It keeps a small state file
under `XDG_STATE_HOME` / `~/.local/state/dotfiles/` so later bootstrap runs can
clean up removed or reshaped managed targets.
...
tracked `home/.vim/autoload/plug.vim` file is the vim-plug loader snapshot.
...
From Vim, **`:PlugUpgrade`** updates `plug.vim`. Because
`~/.vim/autoload/plug.vim` points at the tracked
`home/.vim/autoload/plug.vim` snapshot, the upgrade changes this repo file
directly. Review and commit the snapshot when you want the new version recorded.

...
- Source: `home/.local/bin/make-chrome-app`, `lib/make-chrome-app/makeapp.sh`
...
- Gotchas: macOS-only; requires a valid icon path and write access to `/Applications`; the live CLI is now managed under `home/.local/bin/` and deployed as a symlink in `~/.local/bin/`, while `lib/make-chrome-app/` remains the older vendor/reference implementation.

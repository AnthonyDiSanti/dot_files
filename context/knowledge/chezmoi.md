...
- Why it matters: this repo bootstraps by using chezmoi to manage dotfiles from `home/`, primarily as symlinks back into the repo checkout so `git pull` updates live config.
...
- Key points:
  ...
  - `.chezmoiroot` can let this repo keep `home/` as the source-state root while leaving `settings/`, `lib/`, and `context/` outside target state.
  - This repo passes `--mode symlink` from `bootstrap.sh` so regular non-template dotfiles point back to tracked source files; use `symlink_*.tmpl` when the deployed target itself should be a symlink to a computed path (for example, into a vendored submodule outside `home/`).

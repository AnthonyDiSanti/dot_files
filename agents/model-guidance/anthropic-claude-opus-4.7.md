...
- adaptive thinking plus effort replaces manual thinking budgets
- non-default sampling parameters are removed
- summarized thinking is omitted by default unless explicitly requested
- token counts can differ from Opus 4.6
- high-resolution images are supported and may cost more tokens
- `xhigh` effort exists and is recommended for many coding/agentic use cases
- task budgets exist as an advisory beta for full agentic loops
...
Opus 4.7 uses adaptive thinking when thinking is enabled. Manual thinking budgets
from older models do not carry forward. If a prompt depends on visible thinking
or explicit thinking budgets, update both API parameters and user-facing UX.
...
Codex through home/.codex/skills symlink nodes.
...
- raise `max_tokens` for high/xhigh/max effort workflows
...
Opus 4.7 supports higher-resolution images than prior Claude models. This matters
for computer use, screenshot analysis, document analysis, and UI inspection.
...
Skill-specific model notes belong under `agents/skills/_models/`. Keep this
guide focused on Claude Opus 4.7 behavior that generalizes across skills; use

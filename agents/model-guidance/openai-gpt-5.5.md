...
- Repo consumers:
  - `agents/skills/commit-prep/SKILL.md`
...
- a justification for changing model slugs without checking live docs
...
Skill-specific model notes belong under `agents/skills/_models/`. Keep this guide
focused on GPT-5.5 behavior that generalizes across skills; use per-skill notes
for audits, tuning conclusions, and examples that only apply to one skill.

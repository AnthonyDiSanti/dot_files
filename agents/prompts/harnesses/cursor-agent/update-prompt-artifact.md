...
## Cursor Agent Runner Context

Cursor Agent receives this rendered prompt on stdin in non-interactive print
mode, with the repository root as the working directory. Resolve repo-relative
input and output paths against that working directory.

The harness may pass `--model auto` for account or headless compatibility; that
selector does not name the artifact model. Do not add target-model CLI override
instructions to prompt artifacts, and do not change `agents/harnesses/*.yaml`
from this maintenance pass.

If Cursor Agent cannot complete because of auth, sandboxing, account limits,
model capacity, preview access, or unavailable tools, leave the prompt artifact
missing or stale. Report the blocker and the exact
`agents/scripts/update-prompt.bash` command Anthony should run from a normal
shell.
...
- If the native harness cannot complete because of auth, sandboxing, account
  limits, model capacity, or unavailable preview access, leave the prompt
  artifact missing/stale and report the blocker plus the exact updater command
  for a normal shell run. Do not substitute another harness unless the updater
  or user explicitly selected a fallback.

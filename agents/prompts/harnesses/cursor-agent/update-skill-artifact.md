...
selector does not name the artifact model. Use the local user harness
configuration as the authoring model. Do not add target-model CLI override
instructions to runtime artifacts, and do not change `agents/harnesses/*.yaml`
from this maintenance pass.

If Cursor Agent cannot complete because of auth, sandboxing, account limits,
model capacity, preview access, or unavailable tools, leave the runtime artifact
missing or stale. Report the blocker and the exact
`agents/scripts/update-skill.bash` command Anthony should run from a normal
shell.
...
- Use the local harness configuration as the authoring model. Do not add
  ...
  names a model.
- Root `AGENTS.md` is the shared repo contract for Cursor; avoid duplicating it
  inside runtime `SKILL.md` artifacts.

...
Claude Code receives this rendered prompt on stdin through print mode (`-p`),
with the repository root as the working directory. Resolve repo-relative input
and output paths against that working directory.

Claude Code artifact production uses accept-edits permission mode and the local
user's configured model. Do not add model override instructions to prompt
artifacts, and do not change `agents/harnesses/*.yaml` from this maintenance
pass.

If Claude Code cannot complete because of auth, sandboxing, account limits,
model capacity, preview access, or unavailable tools, leave the prompt artifact
missing or stale. Report the blocker and the exact `agents/scripts/update-prompt.bash`

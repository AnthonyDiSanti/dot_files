...
commit message is grounded in the live dirty tree.
...
Gather enough state to make the commit message and context updates accurate,
then stop. Do not read unrelated files or run additional searches once the dirty
tree is classified:
...
Expand inspection only when evidence is missing, contradictory, or a diff
summary is too coarse to draft an accurate message.
...
Keep context updates compact. Avoid rewriting sections that are already current.

...
- **Vim** uses **[lifepillar/vim-solarized8](https://github.com/lifepillar/vim-solarized8)** via vim-plug (`home/dot_vimrc`).
- **tmux** colors in this repo are inlined in `home/dot_tmux.conf` (Solarized-inspired hex in comments and status bar).
- **Ghostty** Solarized Dark is checked in as `home/dot_config/ghostty/config` (bootstrap links `~/.config/ghostty/config`). Reload Ghostty after changes (e.g. **Cmd+Shift+,**).
